...
from .utils import dim, same_type
import re
...
class property_array(ndarray):
    ...
    **Examples**
    ...
    def __new__(cls, properties, order='C'):
        flat = np.array(properties).flatten()
        same_type(flat, FreeProperty)
    ...
    def __repr__(self):
        ...
        try: 
            units = type(view.item(0)).units
        except IndexError:
            units = ''
        
        if units:
            all_units = np.array([i._units for i in view.flatten()])
            consistent_units = (all_units == units).all()
            if consistent_units: units = dim(f' ({units})')
        
        return array.__repr__(self.astype(type(self.item(0).value))) + units
